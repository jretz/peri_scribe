...
def draw_plot(
    ...
    *,
    ...
    with mpl.rc_context(sns.axes_style("whitegrid")):
        if series_list:
            sns.lineplot(
                data=peri_scribe.kml.plot_data.plot_frame(series_list),
                x=peri_scribe.kml.plot_data.OBSERVATION_TIME_COLUMN,
                y=peri_scribe.kml.plot_data.VALUE_COLUMN,
                hue=peri_scribe.kml.plot_data.LABEL_COLUMN,
                hue_order=labels,
                legend=False,
                ax=axes,
            )
