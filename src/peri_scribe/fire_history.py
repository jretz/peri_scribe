...
def perimeter_row(
    fire: peri_scribe.models.Fire,
    classification: peri_scribe.models.FireClassification | None,
    observation: peri_scribe.perimeter_versions.SourceObservation,
) -> dict[str, object]:
    ...
    row.update(
        {
            ...
            "source_subsource": peri_scribe.history_attributes.text_attribute(
                attributes,
                "source",
                "poly_Source",
            ),
            "source_objectid": observation.object_id,
            "source_globalid": peri_scribe.history_attributes.text_attribute(
                attributes,
                "GlobalID",
            ),
            "source_file": observation.source_file,
            "source_serial": observation.serial_number,
            "observation_time": peri_scribe.perimeter_versions.effective_time(
                observation,
            ),
            ...
            "discovery_time": peri_scribe.history_attributes.datetime_attribute(
                attributes,
                "FireDiscoveryDate",
                "attr_FireDiscoveryDateTime",
            ),
            "area_acres": peri_scribe.history_attributes.float_attribute(
                attributes,
                "area_acres",
                "poly_GISAcres",
            ),
            "percent_contained": peri_scribe.history_attributes.float_attribute(
                attributes,
                "attr_PercentContained",
            ),
            "containment_datetime": peri_scribe.history_attributes.datetime_attribute(
                attributes,
                "attr_ContainmentDateTime",
            ),
            "estimated_cost_to_date": peri_scribe.history_attributes.float_attribute(
                attributes,
                "attr_EstimatedCostToDate",
            ),
            "estimated_final_cost": peri_scribe.history_attributes.float_attribute(
                attributes,
                "attr_EstimatedFinalCost",
            ),
            "type": peri_scribe.history_attributes.text_attribute(attributes, "type"),
            "feature_category": peri_scribe.history_attributes.text_attribute(
                attributes,
                "poly_FeatureCategory",
            ),
            "map_method": peri_scribe.history_attributes.text_attribute(
                attributes,
                "poly_MapMethod",
            ),
            "mission": peri_scribe.history_attributes.text_attribute(
                attributes,
                "mission",
            ),
            "description": peri_scribe.history_attributes.text_attribute(
                attributes,
                "description",
            ),
    ...
    row.update(
        {
            ...
            "incident_size": peri_scribe.history_attributes.float_attribute(
                attributes,
                "IncidentSize",
            ),
            "discovery_acres": peri_scribe.history_attributes.float_attribute(
                attributes,
                "DiscoveryAcres",
            ),
            "final_acres": peri_scribe.history_attributes.float_attribute(
                attributes,
                "FinalAcres",
            ),
            "estimated_cost_to_date": peri_scribe.history_attributes.float_attribute(
            ...
            "control_datetime": peri_scribe.history_attributes.datetime_attribute(
                attributes,
                "ControlDateTime",
            ),
            "source_attributes": attributes_json(attributes),
...
def history_rows_for_fire(
    fire: peri_scribe.models.Fire,
    group: tuple[int, ...],
    full_rows: list[peri_scribe.geo_package.FireRowRecord],
    ...
    *,
    ...
    """
    observations = [
        peri_scribe.perimeter_versions.source_observation_from_row(
            full_rows[index],
            full_paths[index],
            sources_directory,
        )
    ...
    firis_observations = [
        ...
        if observation.source_kind is peri_scribe.perimeter_versions.FIRIS_PERIMETER
    ]
    wfigs_observations = [
        observation
        for observation in observations
        if observation.source_kind is peri_scribe.perimeter_versions.WFIGS_PERIMETER
    ]
    point_observations = [
        observation
        for observation in observations
        if observation.source_kind is peri_scribe.perimeter_versions.WFIGS_LOCATION
    ]
    reconciled_perimeters = peri_scribe.perimeter_versions.reconcile_perimeter_versions(
        peri_scribe.perimeter_versions.collapse_identical_consecutive_perimeters(
            firis_observations,
        ),
        peri_scribe.perimeter_versions.collapse_identical_consecutive_perimeters(
            wfigs_observations,
    ...
    reconciled_perimeters = (
        peri_scribe.perimeter_versions.drop_implausibly_small_perimeters(
            reconciled_perimeters,
        )
    )
    perimeter_rows = [
    ...
    point_rows = [
        ...
        for version in peri_scribe.perimeter_versions.point_versions(point_observations)
    ]
