...
def feature_contents(
    ...
    for row in dataframe.itertuples(index=False, name=None):
        ...
        attributes = tuple(
            peri_scribe.sources.changes.normalized_attribute_value(values[column])
...
def validate_complete_sources(
    ...
    feeds: typing.Iterable[peri_scribe.sources.feed_types.Feed],
    ...
    for feed in feeds:
        complete_path = peri_scribe.sources.snapshots.validation_geopackage_path(
            year_directory,
            feed.name,
        )
        complete_dataframe = peri_scribe.geo.reading.read_layer_dataframe(
            complete_path,
            feed,
        )
        stored_dataframe = peri_scribe.sources.feed_state.existing_features(
