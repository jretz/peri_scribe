...
def test_spatial_reference_wkids_non_dict_is_empty() -> None:
    assert peri_scribe.spatial_reference.spatial_reference_wkids("EPSG:4326") == set()


def test_spatial_reference_wkids_empty_dict_is_empty() -> None:
    assert peri_scribe.spatial_reference.spatial_reference_wkids({}) == set()


def test_spatial_reference_wkids_integer_wkid() -> None:
    assert peri_scribe.spatial_reference.spatial_reference_wkids(
        {"wkid": WGS84_WKID},
    ) == {
        WGS84_WKID,
    }
...
def test_spatial_reference_wkids_non_numeric_string_ignored() -> None:
    assert (
        peri_scribe.spatial_reference.spatial_reference_wkids({"wkid": "abc"}) == set()
    )
...
def test_spatial_reference_wkids_ignores_other_value_types() -> None:
    assert (
        peri_scribe.spatial_reference.spatial_reference_wkids(
...
def test_layer_wkids_no_reported_references_is_empty() -> None:
    ...
    assert peri_scribe.spatial_reference.layer_wkids(layer) == set()


def test_layer_wkids_from_layer_properties() -> None:
    layer = LayerStub(properties={"spatialReference": {"wkid": WGS84_WKID}})
    assert peri_scribe.spatial_reference.layer_wkids(layer) == {WGS84_WKID}
...
def test_layer_wkids_from_extents() -> None:
    ...
    assert peri_scribe.spatial_reference.layer_wkids(layer) == {
        WGS84_WKID,
        NAD83_WKID,
        WEB_MERCATOR_WKID,
    }
...
def test_bounds_of_empty_list_is_none() -> None:
    assert peri_scribe.spatial_reference.bounds_of([]) is None


def test_bounds_of_all_null_geometries_is_none() -> None:
    geometries: list[shapely.Geometry | None] = [None, None]
    assert peri_scribe.spatial_reference.bounds_of(geometries) is None
...
def test_bounds_of_single_point() -> None:
    ...
    assert peri_scribe.spatial_reference.bounds_of(geometries) == (1.0, 1.0, 2.0, 2.0)
...
def test_bounds_of_multiple_points() -> None:
    ...
    assert peri_scribe.spatial_reference.bounds_of(geometries) == (1.0, 3.0, 2.0, 4.0)
...
def test_spatial_reference_domain_geographic() -> None:
    domain = peri_scribe.spatial_reference.spatial_reference_domain(WGS84_WKID)
...
def test_spatial_reference_domain_projected() -> None:
    domain = peri_scribe.spatial_reference.spatial_reference_domain(WEB_MERCATOR_WKID)
    assert domain is not None
...
def test_spatial_reference_domain_unknown_wkid_is_none() -> None:
    assert peri_scribe.spatial_reference.spatial_reference_domain(UNKNOWN_WKID) is None


def test_spatial_reference_domain_vertical_crs_is_none() -> None:
    assert (
        peri_scribe.spatial_reference.spatial_reference_domain(NAVD88_HEIGHT_WKID)
        is None
    )


def test_axis_fits_within_band() -> None:
    assert peri_scribe.spatial_reference.axis_fits(10.0, 20.0, 5.0, 30.0)


def test_axis_fits_exact_boundaries() -> None:
    assert peri_scribe.spatial_reference.axis_fits(5.0, 30.0, 5.0, 30.0)


def test_axis_fits_exceeds_maximum_magnitude() -> None:
    assert not peri_scribe.spatial_reference.axis_fits(10.0, 31.0, 5.0, 30.0)


def test_axis_fits_below_minimum_magnitude() -> None:
    assert not peri_scribe.spatial_reference.axis_fits(4.0, 20.0, 5.0, 30.0)


def test_axis_fits_crosses_zero_with_positive_minimum() -> None:
    assert not peri_scribe.spatial_reference.axis_fits(-5.0, 5.0, 5.0, 30.0)


def test_axis_fits_zero_crossing_allowed_when_minimum_is_zero() -> None:
    assert peri_scribe.spatial_reference.axis_fits(-5.0, 5.0, 0.0, 30.0)
...
def test_coordinates_match_domain_when_all_axes_fit() -> None:
    ...
    assert peri_scribe.spatial_reference.coordinates_match_domain(domain, bounds)


def test_coordinates_match_domain_x_axis_too_small() -> None:
    domain = (1000.0, 20000000.0, 1000.0, 20000000.0)
    bounds = (100.0, 3000000.0, 4000000.0, 5000000.0)
    assert not peri_scribe.spatial_reference.coordinates_match_domain(domain, bounds)
...
def test_longitudes_in_area_inside_normal_area() -> None:
    assert peri_scribe.spatial_reference.longitudes_in_area(
        -124.45,
        -114.12,
        -120.0,
        -119.0,
...
def test_longitudes_in_area_outside_normal_area_west() -> None:
    assert not peri_scribe.spatial_reference.longitudes_in_area(
        -124.45,
        -114.12,
        -125.0,
        -124.0,
...
def test_coordinates_in_area_outside_california_albers() -> None:
    crs = pyproj.CRS.from_epsg(CALIFORNIA_ALBERS_WKID)
    assert not peri_scribe.spatial_reference.coordinates_in_area(
        crs,
        (-99.0, -98.0, 30.0, 31.0),
    )
...
def test_area_of_use_text_formats_bounds() -> None:
    crs = pyproj.CRS.from_epsg(CALIFORNIA_ALBERS_WKID)
    assert (
        peri_scribe.spatial_reference.area_of_use_text(crs)
...
def test_area_of_use_text_unknown() -> None:
    crs = pyproj.CRS.from_proj4("+proj=aeqd +lat_0=0 +lon_0=0 +datum=WGS84 +units=m")
    assert peri_scribe.spatial_reference.area_of_use_text(crs) == "unknown"
...
def test_choose_spatial_reference_id_no_reported_wkids_fails() -> None:
    ...
    with pytest.raises(
        ...
        peri_scribe.spatial_reference.choose_spatial_reference_id(
            layer,
            feature_set,
            None,
        )
...
def test_select_spatial_reference_wkid_no_candidates() -> None:
    selection = peri_scribe.spatial_reference.select_spatial_reference_wkid(set(), None)
