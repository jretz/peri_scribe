...
def test_derive_full_geo_builds_history_for_directory(
    monkeypatch: pytest.MonkeyPatch,
    runner: click.testing.CliRunner,
) -> None:
    ...
    def write_history_of_full_geography(
        ...
        return pathlib.Path("/derived.gpkg")
    ...
    result = runner.invoke(
        peri_scribe.main.cli,
        ["derive-full-geo", "data/2026"],
    )
    assert result.exit_code == 0
    assert calls == [pathlib.Path("data/2026")]


def test_derive_full_geo_defaults_to_current_year_directory(
    monkeypatch: pytest.MonkeyPatch,
    runner: click.testing.CliRunner,
    current_year: typing.Iterator[None],
) -> None:
    ...
    result = runner.invoke(peri_scribe.main.cli, ["derive-full-geo"])
    assert result.exit_code == 0
