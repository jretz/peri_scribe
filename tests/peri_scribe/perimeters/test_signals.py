...
@hypothesis.given(
    geometry=tests.geometry_strategies.local_shapes(),
...
@hypothesis.given(
    scenario=tests.peri_scribe.perimeters.classification_helpers.extent_histories(),
)
def test_extent_signal_ignores_older_observations_and_history_order(
    ...
    assert peri_scribe.perimeters.signals.extent_signal(history, CONFIG) == (
        peri_scribe.perimeters.signals.extent_signal(latest, CONFIG)
    )
