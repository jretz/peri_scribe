...
def band(name: str) -> peri_scribe.perimeters.progression.ProgressionBand:
    ...
    Returns:
        The band.
    """
    for candidate in peri_scribe.perimeters.progression.PROGRESSION_BANDS:
...
def test_ring_date_converts_to_california_date() -> None:
    ...
    assert peri_scribe.perimeters.progression.ring_date(observation_time) == (
        datetime.date(2026, 8, 5)
    )


def test_band_for_age_covers_every_age() -> None:
    assert peri_scribe.perimeters.progression.band_for_age(0) is band("Latest Day")
    assert peri_scribe.perimeters.progression.band_for_age(1) is band(
        "2 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(2) is band(
        "2 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(3) is band(
        "4 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(6) is band(
        "4 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(7) is band(
        "8 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(14) is band(
        "8 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(15) is band(
        "16 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(30) is band(
        "16 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(31) is band(
        "32 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(62) is band(
        "32 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(63) is band(
        "64 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(126) is band(
        "64 Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(127) is band(
        "128+ Days Before That",
    )
    assert peri_scribe.perimeters.progression.band_for_age(365) is band(
        "128+ Days Before That",
    )


def test_band_for_age_returns_none_without_band() -> None:
    assert peri_scribe.perimeters.progression.band_for_age(-1) is None


def test_band_label_names_single_date() -> None:
    latest_date = datetime.date(2026, 8, 15)
    assert (
        peri_scribe.perimeters.progression.band_label(
            band("Latest Day"),
            latest_date,
...
def test_progression_bands_groups_rings_into_bands() -> None:
    bands = peri_scribe.perimeters.progression.progression_bands(
        (
            ring(
                square(1.0),
...
def test_progression_bands_returns_nothing_without_rings() -> None:
    assert peri_scribe.perimeters.progression.progression_bands(()) == ()
...
def test_progression_band_rings_skips_undated_rings() -> None:
    assert (
        peri_scribe.perimeters.progression.progression_band_rings(
            (ring(square(1.0)),),
        )
        == ()
    )
...
def test_progression_band_rings_returns_nothing_without_rings() -> None:
    assert peri_scribe.perimeters.progression.progression_band_rings(()) == ()
