...
import tests.peri_scribe.areas_helpers
from peri_scribe.units import units


@hypothesis.given(
    mapping_entries=tests.peri_scribe.areas_helpers.history_entries(),
    report_entries=tests.peri_scribe.areas_helpers.history_entries(),
)
def test_area_history_never_uses_future_evidence(
    ...
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings(mapping_entries),
        tests.peri_scribe.areas_helpers.reports(report_entries, confirmed=True),
    )
...
def test_area_history_is_independent_of_input_row_order(
    ...
    perimeters = tests.peri_scribe.areas_helpers.mappings(mapping_entries)
    reports = tests.peri_scribe.areas_helpers.reports(report_entries, confirmed=True)
...
@hypothesis.given(entries=tests.peri_scribe.areas_helpers.history_entries())
def test_area_history_preserves_confirmed_reported_sizes(
    entries: list[tuple[float, float]],
) -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([]),
        tests.peri_scribe.areas_helpers.reports(entries, confirmed=True),
    )
...
def test_area_history_prefers_recent_geometry_over_conflicting_reports() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 1000)]),
        tests.peri_scribe.areas_helpers.reports([(1, 1200), (2, 900)]),
    )
    assert all(
        tests.peri_scribe.areas_helpers.acres(item.area) == pytest.approx(1000)
        for item in history
    )
...
def test_area_history_uses_reports_without_mapping() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([]),
        tests.peri_scribe.areas_helpers.reports([(0, 25), (1, 50)]),
    )
...
def test_area_history_switches_for_significant_growth_after_stale_deadline() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 1000)]),
        tests.peri_scribe.areas_helpers.reports([(2, 1500), (4, 1500)]),
    )
    assert tests.peri_scribe.areas_helpers.acres(
        next(
            item
            for item in history
            if item.time == tests.peri_scribe.areas_helpers.time(3)
        ).area,
    ) == pytest.approx(1500)
    assert history[-1].observation_time == tests.peri_scribe.areas_helpers.time(4)
...
def test_area_history_keeps_old_mapping_without_significant_growth(area: float) -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.latest_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 1000)]),
            tests.peri_scribe.areas_helpers.reports([(5, area)]),
        ),
    ) == pytest.approx(1000)
...
def test_area_history_requires_absolute_increase_for_small_fire() -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.latest_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 10)]),
            tests.peri_scribe.areas_helpers.reports([(5, 15)]),
        ),
    ) == pytest.approx(10)
...
def test_area_history_requires_growth_since_mapping() -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.latest_area(
            tests.peri_scribe.areas_helpers.mappings([(1, 1000)]),
            tests.peri_scribe.areas_helpers.reports([(0, 2000), (5, 2000)]),
        ),
    ) == pytest.approx(1000)


def test_area_history_uses_dispatch_growth_while_a_formal_report_is_unchanged() -> None:
    points = tests.peri_scribe.areas_helpers.reports(
        [(4, 1200), (5, 3171)],
        confirmed=True,
    )
    points.loc[1, "source_attributes"] = json.dumps({
        "ModifiedBySystem": "wildcade",
        "ICS209ReportDateTime": tests.peri_scribe.areas_helpers.time(4).isoformat(),
    })
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 1163)]),
        points,
    )
    assert history[-1].source is peri_scribe.areas.AreaSource.REPORTED
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(
        3171,
    )
...
def test_area_history_accepts_two_fresh_reports_for_rapid_growth() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 400)]),
        tests.peri_scribe.areas_helpers.reports([(1, 900), (2, 1300)], confirmed=True),
    )
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(
        1300,
    )
...
def test_area_history_does_not_refresh_mapping_for_negligible_geometry_edit() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 17), (5, 17.001)]),
        tests.peri_scribe.areas_helpers.reports([(4, 500)]),
    )
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(500)
    assert history[-1].source is peri_scribe.areas.AreaSource.REPORTED


def test_area_history_resumes_geometry_when_mapping_catches_up() -> None:
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 17), (6, 575)]),
        tests.peri_scribe.areas_helpers.reports([(4, 500)]),
    )
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(575)
...
def test_area_history_accepts_new_survey_of_unchanged_boundary() -> None:
    perimeters = tests.peri_scribe.areas_helpers.mappings([(0, 1000), (5, 1000)])
    perimeters["source_subsource"] = ["FIRIS", "CAL FIRE INTEL FLIGHT DATA"]
    history = peri_scribe.areas.area_history(
        perimeters,
        tests.peri_scribe.areas_helpers.reports([(4, 2000)]),
    )
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(
        1000,
    )
...
def test_area_history_uses_independent_incident_history() -> None:
    incident_rows = tests.peri_scribe.areas_helpers.reports([(5, 1500)])
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.latest_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 1000)]),
            tests.peri_scribe.areas_helpers.reports([]),
            incident_rows,
        ),
    ) == pytest.approx(1500)
...
def test_area_history_preserves_downward_mapping_corrections() -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.latest_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 1000), (5, 700)]),
            tests.peri_scribe.areas_helpers.reports([]),
        ),
    ) == pytest.approx(700)
...
def test_area_history_ignores_empty_and_undated_geometry() -> None:
    perimeters = tests.peri_scribe.areas_helpers.mappings([(0, 1000), (1, 1100)])
...
def test_area_history_supports_configured_staleness() -> None:
    ...
    history = peri_scribe.areas.area_history(
        tests.peri_scribe.areas_helpers.mappings([(0, 1000)]),
        tests.peri_scribe.areas_helpers.reports([(5, 1500)]),
        policy=policy,
    )
    assert tests.peri_scribe.areas_helpers.acres(history[-1].area) == pytest.approx(
        1000,
...
def test_new_survey_rejects_unusable_capture_dates(capture: str | None) -> None:
    row = pd.Series({
        "observation_time": tests.peri_scribe.areas_helpers.time(5),
        "source_attributes": {"poly_PolygonDateTime": capture},
    })
    assert not peri_scribe.areas.new_survey(
        row,
        tests.peri_scribe.areas_helpers.time(0),
    )
...
def test_new_survey_accepts_advanced_capture_date() -> None:
    row = pd.Series({
        "observation_time": tests.peri_scribe.areas_helpers.time(5),
        "source_attributes": {
            "poly_PolygonDateTime": tests.peri_scribe.areas_helpers.time(4),
        },
    })
    assert peri_scribe.areas.new_survey(row, tests.peri_scribe.areas_helpers.time(0))
...
def test_accepted_reports_rejects_uncorroborated_large_decrease() -> None:
    updates = peri_scribe.incidents.history(
        tests.peri_scribe.areas_helpers.mappings([]),
        tests.peri_scribe.areas_helpers.reports([(0, 39000), (1, 4000)]),
    )
...
def test_accepted_reports_allows_explicit_report_correction() -> None:
    updates = peri_scribe.incidents.history(
        tests.peri_scribe.areas_helpers.mappings([]),
        tests.peri_scribe.areas_helpers.reports([(0, 1000), (1, 700)], confirmed=True),
    )
    assert (
...
def test_latest_area_uses_undated_geometry() -> None:
    perimeters = tests.peri_scribe.areas_helpers.mappings([(0, 1000)])
...
def test_row_area_skips_zero_measurement() -> None:
    row = tests.peri_scribe.areas_helpers.mappings([(0, 1)]).iloc[0].copy()
...
def test_historical_area_keeps_earlier_size_after_mapping_correction() -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.historical_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 100), (4, 10)]),
            tests.peri_scribe.areas_helpers.reports([]),
        ),
    ) == pytest.approx(100)
...
def test_historical_area_rejects_report_conflicting_with_fresh_mapping() -> None:
    assert tests.peri_scribe.areas_helpers.acres(
        peri_scribe.areas.historical_area(
            tests.peri_scribe.areas_helpers.mappings([(0, 10)]),
            tests.peri_scribe.areas_helpers.reports([(1, 100)]),
        ),
    ) == pytest.approx(10)


def test_historical_area_prefers_undated_geometry_to_supplied_acreage() -> None:
    perimeters = tests.peri_scribe.areas_helpers.mappings([(0, 30)])
    perimeters["observation_time"] = None
