"""Tests for peri_scribe.kml.row_values."""
...
def test_column_value_returns_none_for_missing_row_or_column() -> None:
    ...
    assert peri_scribe.kml.row_values.column_value(row, "not_a_column") is None
    assert peri_scribe.kml.row_values.column_value(None, "fire_name") is None
...
def test_text_value_returns_none_for_blank() -> None:
    ...
    assert peri_scribe.kml.row_values.text_value(row, "fire_name") == "Bug"
...
def test_as_datetime_parses_strings_and_timestamps() -> None:
    ...
    assert peri_scribe.kml.row_values.as_datetime("2026-08-05T20:30:00Z") == expected
    assert (
        peri_scribe.kml.row_values.as_datetime(pd.Timestamp("2026-08-05T20:30:00Z"))
        == expected
    )
    assert peri_scribe.kml.row_values.as_datetime("not a date") is None
...
def test_datetime_value_reads_a_column() -> None:
    ...
    assert (
        peri_scribe.kml.row_values.datetime_value(frame.iloc[0], "observation_time")
        == expected
    )
    assert peri_scribe.kml.row_values.datetime_value(None, "observation_time") is None
...
def test_source_attribute_value_reads_json() -> None:
    ...
    assert (
        peri_scribe.kml.row_values.source_attribute_value(frame.iloc[0], "Missing")
        is None
    )
    assert (
        peri_scribe.kml.row_values.source_attribute_value(None, "POOJurisdictionalUnit")
        is None
    )
...
def test_source_text_value_returns_none_for_blank() -> None:
    ...
    assert (
        peri_scribe.kml.row_values.source_text_value(frame.iloc[0], "Missing") is None
    )
...
def test_numbered_source_text_orders_by_slot_number_and_dedupes() -> None:
    ...
    assert (
        peri_scribe.kml.row_values.numbered_source_text(
            perimeter.iloc[0],
            point.iloc[0],
            peri_scribe.kml.text.FIRE_BEHAVIOR_ATTRIBUTE_KEYS,
        )
...
def test_source_label_names_known_sources() -> None:
    assert peri_scribe.kml.row_values.source_label("firis_perimeter") == "FIRIS / NIFC"
    assert peri_scribe.kml.row_values.source_label("wfigs_perimeter") == "WFIGS"
    assert peri_scribe.kml.row_values.source_label("unknown") is None
    assert peri_scribe.kml.row_values.source_label(None) is None
...
def test_source_attributes_dictionary_rejects_missing_or_invalid_values() -> None:
    assert peri_scribe.kml.row_values.source_attributes_dictionary(None) is None
    assert peri_scribe.kml.row_values.source_attributes_dictionary("not json") is None
    assert (
        peri_scribe.kml.row_values.source_attributes_dictionary(json.dumps([1, 2]))
        is None
    )
...
def test_source_attribute_number_reads_numeric_attributes() -> None:
    ...
    assert peri_scribe.kml.row_values.source_attribute_number(row, "Unit") is None
    assert peri_scribe.kml.row_values.source_attribute_number(row, "Missing") is None
    assert peri_scribe.kml.row_values.source_attribute_number(None, "Unit") is None
