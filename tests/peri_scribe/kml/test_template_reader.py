"""Tests for peri_scribe.kml.template_reader."""
...
def test_placemark_style_urls_descends_into_folders() -> None:
    ...
    urls = peri_scribe.kml.template_reader.placemark_style_urls(document)
...
def test_collect_placemark_style_urls_skips_placemarks_without_style() -> None:
    ...
    urls: dict[str, str] = {}
    peri_scribe.kml.template_reader.collect_placemark_style_urls(document, urls)
    assert urls == {}
...
def test_style_from_requires_style_id() -> None:
    ...
    style_element = document.find(tests.peri_scribe.kml.kml_helpers.kml_tag("Style"))
    assert style_element is not None
    with pytest.raises(ValueError, match="no id attribute"):
        peri_scribe.kml.template_reader.style_from(style_element)
...
def test_template_from_requires_document() -> None:
    ...
    with pytest.raises(ValueError, match="no Document element"):
        peri_scribe.kml.template_reader.template_from(kml_text)


def test_read_template_reads_file(monkeypatch: pytest.MonkeyPatch) -> None:
    text = peri_scribe.kml.template.template_kml()
    ...
    assert (
        template.style_urls
        == peri_scribe.kml.template_reader.template_from(text).style_urls
    )
