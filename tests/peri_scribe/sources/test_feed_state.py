"""Tests for peri_scribe.sources.feed_state."""

from __future__ import annotations

import contextlib
import datetime
import pathlib
import sqlite3
import typing

import hypothesis
import pytest
import shapely

import peri_scribe.geo.reading
import peri_scribe.models
import peri_scribe.output
import peri_scribe.sources.feed_state
import peri_scribe.sources.snapshots
import tests.factories
import tests.peri_scribe.sources.changes_helpers
import tests.peri_scribe.sources.feed_state_helpers


if typing.TYPE_CHECKING:
    import geopandas


@hypothesis.given(value=tests.peri_scribe.sources.feed_state_helpers.sql_values())
def test_sql_literal_round_trips_through_a_sql_parser(
    *,
    value: str | float | bool,
) -> None:
    literal = peri_scribe.sources.feed_state.sql_literal(value)
    with contextlib.closing(sqlite3.connect(":memory:")) as database:
        assert database.execute("SELECT " + literal).fetchone() == (value,)


def test_sql_literal_preserves_fractional_values() -> None:
    assert peri_scribe.sources.feed_state.sql_literal(1.5) == "1.5"


def test_sql_literal_escapes_an_embedded_quote() -> None:
    assert peri_scribe.sources.feed_state.sql_literal("'") == "''''"


@hypothesis.given(
    batches=tests.peri_scribe.sources.feed_state_helpers.feature_batches(),
)
def test_latest_features_by_object_id_matches_last_observation_model(
    batches: list[list[tuple[int, str]]],
) -> None:
    frames = tests.peri_scribe.sources.feed_state_helpers.feature_frames(batches)
    expected = dict(entry for batch in batches for entry in batch)
    actual = peri_scribe.sources.feed_state.latest_features_by_object_id(frames)
    assert actual is not None
    assert len(actual) == len(expected)
    assert dict(zip(actual.OBJECTID, actual.value, strict=True)) == expected
    for identifier, geometry in zip(actual.OBJECTID, actual.geometry, strict=True):
        assert geometry == shapely.Point(identifier, len(expected[identifier]))


def test_existing_features_returns_none_without_files(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    feed = tests.factories.change_feed()
    monkeypatch.setattr(
        peri_scribe.sources.snapshots,
        "existing_source_files",
        lambda _directory: [],
    )
    assert (
        peri_scribe.sources.feed_state.existing_features(pathlib.Path("/sources"), feed)
        is None
    )


def test_existing_features_returns_none_without_object_id_column(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    feed = tests.factories.change_feed()
    monkeypatch.setattr(
        peri_scribe.sources.snapshots,
        "existing_source_files",
        lambda _directory: [
            peri_scribe.sources.snapshots.SourceFile(
                serial_number=0,
                last_edit_timestamp=0,
            ),
        ],
    )
    monkeypatch.setattr(
        peri_scribe.geo.reading,
        "read_layer_dataframe",
        lambda _path, _feed: tests.factories.change_dataframe([
            tests.peri_scribe.sources.changes_helpers.SAMPLE_FEATURE_ROW,
        ]).drop(columns=["OBJECTID"]),
    )
    assert (
        peri_scribe.sources.feed_state.existing_features(pathlib.Path("/sources"), feed)
        is None
    )


def test_stored_object_ids_returns_empty_without_existing() -> None:
    assert peri_scribe.sources.feed_state.stored_object_ids(None) == set()


def test_stored_object_ids_returns_empty_without_object_id_column() -> None:
    existing = typing.cast(
        "geopandas.GeoDataFrame",
        tests.factories.change_dataframe([
            tests.peri_scribe.sources.changes_helpers.SAMPLE_FEATURE_ROW,
        ]).drop(columns=["OBJECTID"]),
    )
    assert peri_scribe.sources.feed_state.stored_object_ids(existing) == set()


def test_stored_object_ids_returns_object_ids() -> None:
    existing = tests.factories.change_dataframe([
        tests.peri_scribe.sources.changes_helpers.SAMPLE_FEATURE_ROW,
        (2, "b", (1.0, 1.0)),
    ])
    assert peri_scribe.sources.feed_state.stored_object_ids(existing) == {1, 2}


def test_sql_literal_quotes_text() -> None:
    assert peri_scribe.sources.feed_state.sql_literal("Inactive") == "'Inactive'"


def test_sql_literal_formats_booleans() -> None:
    assert peri_scribe.sources.feed_state.sql_literal(value=True) == "true"
    assert peri_scribe.sources.feed_state.sql_literal(value=False) == "false"


def test_sql_literal_formats_numbers() -> None:
    assert peri_scribe.sources.feed_state.sql_literal(0) == "0"
    assert peri_scribe.sources.feed_state.sql_literal(1.0) == "1"


def test_sql_literal_raises_for_unsupported_type() -> None:
    with pytest.raises(ValueError, match="Unsupported SQL literal"):
        peri_scribe.sources.feed_state.sql_literal(object())


def test_stored_status_object_ids_returns_empty_without_existing() -> None:
    feed = tests.factories.change_feed()
    assert (
        peri_scribe.sources.feed_state.stored_status_object_ids(
            None,
            feed,
            peri_scribe.models.FireStatus.ACTIVE,
        )
        == []
    )


def test_stored_status_object_ids_returns_empty_without_status_column() -> None:
    feed = tests.factories.change_feed()
    existing = tests.factories.change_dataframe([
        tests.peri_scribe.sources.changes_helpers.SAMPLE_FEATURE_ROW,
    ])
    assert (
        peri_scribe.sources.feed_state.stored_status_object_ids(
            existing,
            feed,
            peri_scribe.models.FireStatus.ACTIVE,
        )
        == []
    )


def test_stored_status_object_ids_returns_empty_without_object_id_column() -> None:
    feed = tests.factories.change_feed()
    existing = tests.peri_scribe.sources.changes_helpers.status_dataframe([
        (1, "Active", (0.0, 0.0)),
    ]).drop(columns=["OBJECTID"])
    assert (
        peri_scribe.sources.feed_state.stored_status_object_ids(
            typing.cast("geopandas.GeoDataFrame", existing),
            feed,
            peri_scribe.models.FireStatus.ACTIVE,
        )
        == []
    )


def test_stored_status_object_ids_selects_status_and_ignores_unknown() -> None:
    feed = tests.factories.change_feed()
    existing = tests.peri_scribe.sources.changes_helpers.status_dataframe([
        (1, "Active", (0.0, 0.0)),
        (2, "Inactive", (1.0, 1.0)),
        (3, "Active", (2.0, 2.0)),
        (4, "unknown-status", (3.0, 3.0)),
    ])
    assert peri_scribe.sources.feed_state.stored_status_object_ids(
        existing,
        feed,
        peri_scribe.models.FireStatus.ACTIVE,
    ) == [1, 3]
    assert peri_scribe.sources.feed_state.stored_status_object_ids(
        existing,
        feed,
        peri_scribe.models.FireStatus.INACTIVE,
    ) == [2]


def test_stored_status_literals_returns_empty_without_existing() -> None:
    feed = tests.factories.change_feed()
    assert (
        peri_scribe.sources.feed_state.stored_status_literals(
            None,
            feed,
            peri_scribe.models.FireStatus.INACTIVE,
        )
        == ()
    )


def test_stored_status_literals_returns_empty_without_status_column() -> None:
    feed = tests.factories.change_feed()
    existing = tests.factories.change_dataframe([
        tests.peri_scribe.sources.changes_helpers.SAMPLE_FEATURE_ROW,
    ])
    assert (
        peri_scribe.sources.feed_state.stored_status_literals(
            existing,
            feed,
            peri_scribe.models.FireStatus.INACTIVE,
        )
        == ()
    )


def test_stored_status_literals_distinct_in_first_seen_order() -> None:
    feed = tests.factories.change_feed()
    existing = tests.peri_scribe.sources.changes_helpers.status_dataframe([
        (1, "Inactive", (0.0, 0.0)),
        (2, "Active", (1.0, 1.0)),
        (3, "Inactive", (2.0, 2.0)),
        (4, "0", (3.0, 3.0)),
        (5, 0, (4.0, 4.0)),
        (6, False, (5.0, 5.0)),
        (7, "unknown-status", (6.0, 6.0)),
    ])
    assert peri_scribe.sources.feed_state.stored_status_literals(
        existing,
        feed,
        peri_scribe.models.FireStatus.INACTIVE,
    ) == ("'Inactive'", "'0'", "0", "false")


def test_stored_status_literals_selects_active_values() -> None:
    feed = tests.factories.change_feed()
    existing = tests.peri_scribe.sources.changes_helpers.status_dataframe([
        (1, "Active", (0.0, 0.0)),
        (2, 1, (1.0, 1.0)),
    ])
    assert peri_scribe.sources.feed_state.stored_status_literals(
        existing,
        feed,
        peri_scribe.models.FireStatus.ACTIVE,
    ) == ("'Active'", "1")


def test_where_clause_for_formats_cutoff() -> None:
    cutoff = datetime.datetime(
        2026,
        1,
        1,
        0,
        0,
        0,
        tzinfo=tests.peri_scribe.sources.changes_helpers.UTC,
    )
    result = peri_scribe.sources.feed_state.where_clause_for(
        ("ModifiedOnDateTime_dt",),
        cutoff,
    )
    assert result == (
        "ModifiedOnDateTime_dt >= timestamp '2026-01-01T00:00:00Z' "
        "OR ModifiedOnDateTime_dt IS NULL"
    )


def test_where_clause_for_combines_multiple_change_columns() -> None:
    cutoff = datetime.datetime(
        2026,
        1,
        1,
        0,
        0,
        0,
        tzinfo=tests.peri_scribe.sources.changes_helpers.UTC,
    )
    result = peri_scribe.sources.feed_state.where_clause_for(
        ("ModifiedOnDateTime_dt", "poly_DateCurrent", "poly_CreateDate"),
        cutoff,
    )
    assert result == (
        "ModifiedOnDateTime_dt >= timestamp '2026-01-01T00:00:00Z' "
        "OR ModifiedOnDateTime_dt IS NULL "
        "OR poly_DateCurrent >= timestamp '2026-01-01T00:00:00Z' "
        "OR poly_DateCurrent IS NULL "
        "OR poly_CreateDate >= timestamp '2026-01-01T00:00:00Z' "
        "OR poly_CreateDate IS NULL"
    )


def test_where_clause_for_includes_null_modified_timestamps() -> None:
    cutoff = datetime.datetime(
        2026,
        1,
        1,
        0,
        0,
        0,
        tzinfo=tests.peri_scribe.sources.changes_helpers.UTC,
    )
    result = peri_scribe.sources.feed_state.where_clause_for(("EditDate",), cutoff)
    assert "EditDate IS NULL" in result


def test_latest_snapshot_path_returns_none_without_files() -> None:
    assert (
        peri_scribe.sources.feed_state.latest_snapshot_path(pathlib.Path("/d"), [])
        is None
    )


def test_latest_snapshot_path_returns_last_file() -> None:
    result = peri_scribe.sources.feed_state.latest_snapshot_path(
        pathlib.Path("/d"),
        [
            peri_scribe.sources.snapshots.SourceFile(
                serial_number=0,
                last_edit_timestamp=0,
            ),
            peri_scribe.sources.snapshots.SourceFile(
                serial_number=1,
                last_edit_timestamp=0,
            ),
        ],
    )
    assert result == pathlib.Path("/d/000___/000001,lastEdit=0.gpkg")


def test_current_state_serial_number_returns_none_without_snapshots(
    tmp_path: pathlib.Path,
) -> None:
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    assert peri_scribe.sources.feed_state.current_state_serial_number(directory) is None


def test_read_current_features_reads_snapshots_without_state_file(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0)), (2, "b", (1.0, 1.0))],
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        1,
        [(2, "B", (1.0, 1.0)), (3, "c", (2.0, 2.0))],
    )
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["OBJECTID"]) == [1, 2, 3]
    assert list(existing["name"]) == ["a", "B", "c"]


def test_read_current_features_uses_fresh_state_file(tmp_path: pathlib.Path) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0)), (2, "b", (1.0, 1.0))],
    )
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(2, "B", (1.0, 1.0)), (3, "c", (2.0, 2.0))]),
    )
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["OBJECTID"]) == [1, 2, 3]
    assert list(existing["name"]) == ["a", "B", "c"]


def test_read_current_features_rebuilds_when_state_is_stale(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0))],
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        1,
        [(2, "b", (1.0, 1.0))],
    )
    # A state file covering only the first snapshot, with stale content.
    state_path = peri_scribe.sources.snapshots.current_state_path(directory, 0)
    state_path.parent.mkdir(parents=True, exist_ok=True)
    peri_scribe.output.write_geopackage(
        state_path,
        [
            peri_scribe.models.LayerData(
                name=feed.name,
                dataframe=tests.factories.change_dataframe([(1, "old", (0.0, 0.0))]),
            ),
        ],
    )
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["OBJECTID"]) == [1, 2]


def test_read_current_features_falls_back_when_state_unreadable(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0)), (2, "b", (1.0, 1.0))],
    )
    state_path = peri_scribe.sources.snapshots.current_state_path(directory, 0)
    state_path.parent.mkdir(parents=True, exist_ok=True)
    state_path.write_bytes(b"not a geopackage")
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["OBJECTID"]) == [1, 2]


def test_write_current_state_merges_and_removes_older_state_files(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0)), (2, "b", (1.0, 1.0))],
    )
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(2, "B", (1.0, 1.0))]),
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        1,
        [(3, "c", (2.0, 2.0))],
    )
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(3, "C", (2.0, 2.0))]),
    )
    state_files = peri_scribe.sources.snapshots.current_state_file_paths(directory)
    assert [serial for serial, _path in state_files] == [1]
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["name"]) == ["a", "B", "C"]


def test_latest_features_by_object_id_normalizes_geometry_column_name() -> None:
    stored = tests.factories.change_dataframe([(1, "a", (0.0, 0.0))])
    fetched = tests.factories.change_dataframe([(2, "b", (1.0, 1.0))]).rename_geometry(
        "geom",
    )
    merged = peri_scribe.sources.feed_state.latest_features_by_object_id([
        stored,
        fetched,
    ])
    assert merged is not None
    assert merged.geometry.name == "geometry"
    assert list(merged["OBJECTID"]) == [1, 2]


def test_write_current_state_merges_fetched_geometry_column(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0))],
    )
    fetched = typing.cast(
        "geopandas.GeoDataFrame",
        tests.factories.change_dataframe([(2, "b", (1.0, 1.0))]).rename_geometry(
            "geom",
        ),
    )
    peri_scribe.sources.feed_state.write_current_state(directory, feed, fetched)
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["OBJECTID"]) == [1, 2]
    assert list(existing["name"]) == ["a", "b"]


def test_write_current_state_skips_without_snapshots(tmp_path: pathlib.Path) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(1, "a", (0.0, 0.0))]),
    )
    assert peri_scribe.sources.snapshots.current_state_file_paths(directory) == []


def test_write_current_state_skips_without_object_id_column(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    # The snapshot and the new rows both lack an OBJECTID column, so no state keyed by
    # OBJECTID can be maintained and nothing is written.
    relative_path = peri_scribe.sources.snapshots.SourceFile(
        serial_number=0,
        last_edit_timestamp=0,
    ).relative_path
    path = directory / relative_path
    path.parent.mkdir(parents=True, exist_ok=True)
    peri_scribe.output.write_geopackage(
        path,
        [
            peri_scribe.models.LayerData(
                name=feed.name,
                dataframe=typing.cast(
                    "geopandas.GeoDataFrame",
                    tests.factories.change_dataframe([(1, "a", (0.0, 0.0))]).drop(
                        columns=["OBJECTID"],
                    ),
                ),
            ),
        ],
    )
    frame = typing.cast(
        "geopandas.GeoDataFrame",
        tests.factories.change_dataframe([(2, "b", (1.0, 1.0))]).drop(
            columns=["OBJECTID"],
        ),
    )
    peri_scribe.sources.feed_state.write_current_state(directory, feed, frame)
    assert peri_scribe.sources.snapshots.current_state_file_paths(directory) == []


def test_write_current_state_rebuilds_from_snapshots_when_state_unreadable(
    tmp_path: pathlib.Path,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0)), (2, "b", (1.0, 1.0))],
    )
    state_path = peri_scribe.sources.snapshots.current_state_path(directory, 0)
    state_path.parent.mkdir(parents=True, exist_ok=True)
    state_path.write_bytes(b"not a geopackage")
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(3, "c", (2.0, 2.0))]),
    )
    existing = peri_scribe.sources.feed_state.read_current_features(directory, feed)
    assert existing is not None
    assert list(existing["name"]) == ["a", "b", "c"]


def test_write_current_state_ignores_missing_old_state_file(
    tmp_path: pathlib.Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    feed = tests.factories.change_feed()
    directory = tests.peri_scribe.sources.changes_helpers.snapshot_source_directory(
        tmp_path,
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        0,
        [(1, "a", (0.0, 0.0))],
    )
    tests.peri_scribe.sources.changes_helpers.write_snapshot(
        directory,
        feed,
        1,
        [(2, "b", (1.0, 1.0))],
    )
    stale_path = peri_scribe.sources.snapshots.current_state_path(directory, 0)
    current_path = peri_scribe.sources.snapshots.current_state_path(directory, 1)
    stale_path.parent.mkdir(parents=True, exist_ok=True)
    for path in (stale_path, current_path):
        peri_scribe.output.write_geopackage(
            path,
            [
                peri_scribe.models.LayerData(
                    name=feed.name,
                    dataframe=tests.factories.change_dataframe([
                        (9, "stale", (9.0, 9.0)),
                    ]),
                ),
            ],
        )
    original_unlink = pathlib.Path.unlink

    flaky_unlink = (
        tests.peri_scribe.sources.changes_helpers.make_disappearing_state_unlink(
            stale_path=stale_path,
            original_unlink=original_unlink,
        )
    )

    monkeypatch.setattr(pathlib.Path, "unlink", flaky_unlink)
    peri_scribe.sources.feed_state.write_current_state(
        directory,
        feed,
        tests.factories.change_dataframe([(2, "B", (1.0, 1.0))]),
    )
    state_files = peri_scribe.sources.snapshots.current_state_file_paths(directory)
    assert [serial for serial, _path in state_files] == [1]
