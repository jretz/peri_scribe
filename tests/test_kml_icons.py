"""Tests for peri_scribe.kml_icons."""
...
def test_progression_icon_filename_names_the_band() -> None:
    assert (
        peri_scribe.kml_icons.progression_icon_filename(0) == "progression-band-1.png"
    )
    assert (
        peri_scribe.kml_icons.progression_icon_filename(7) == "progression-band-8.png"
    )


def test_interior_icon_filename_names_the_folder() -> None:
    assert peri_scribe.kml_icons.interior_icon_filename() == "interior.png"


def test_perimeters_icon_filename_names_the_folder() -> None:
    assert peri_scribe.kml_icons.perimeters_icon_filename() == "perimeters.png"
...
def test_perimeters_icon_draws_two_full_width_lines() -> None:
    ...
    rows = tests.kml_helpers.png_pixel_rows(
        peri_scribe.kml_icons.perimeters_icon(template),
    )
    side = peri_scribe.kml_icons.PROGRESSION_ICON_SIDE_LENGTH_IN_PIXELS
...
def test_kml_color_rgb_decodes_aabbggrr() -> None:
    assert peri_scribe.kml_icons.kml_color_rgb("7f002aff") == (255, 42, 0)
    assert peri_scribe.kml_icons.kml_color_rgb("7fbdb7b0") == (176, 183, 189)


def test_solid_color_png_fills_a_square() -> None:
    content = peri_scribe.kml_icons.solid_color_png(1, 2, 3, 4)
    assert tests.kml_helpers.png_color(content) == (1, 2, 3)
...
def test_interior_icon_matches_template_color() -> None:
    ...
    assert tests.kml_helpers.png_color(
...
def test_progression_icons_match_template_colors() -> None:
    ...
    icons = peri_scribe.kml_icons.progression_icons(template)
    assert set(icons) == {
    ...
    assert tests.kml_helpers.png_color(icons["progression-band-1.png"]) == (255, 42, 0)
    assert tests.kml_helpers.png_color(icons["progression-band-8.png"]) == (
        176,
        183,
        189,
    )
