"""Tests for peri_scribe.source_validation."""
...
def validation_feed(index: int) -> peri_scribe.feed_types.ArcGISFeed:
...
def test_validate_feed_clean_when_stored_covers_complete() -> None:
    ...
    result = peri_scribe.source_validation.validate_feed(
        change_feed(),
        complete,
        complete,
    )
...
def test_validate_complete_sources_validates_each_feed_in_order(
    ...
    read_calls: list[tuple[pathlib.Path, peri_scribe.feed_types.Feed]] = []
