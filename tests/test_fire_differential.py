"""Tests for peri_scribe.fire_differential."""
...
def full_perimeter_frame(
    ...
    columns = [
        column
        for column in peri_scribe.fire_history.PERIMETER_COLUMNS
        if column != "geometry"
    ]
...
def test_differential_geopackage_path_names_output() -> None:
    assert peri_scribe.fire_differential.differential_geopackage_path(
...
def test_polygonal_area_returns_none_for_missing() -> None:
    assert peri_scribe.fire_differential.polygonal_area(None) is None


def test_polygonal_area_returns_none_for_non_polygonal() -> None:
    line = shapely.geometry.LineString([(0, 0), (1, 1)])
    assert peri_scribe.fire_differential.polygonal_area(line) is None
...
def test_polygonal_area_returns_none_for_line_only_collection() -> None:
    ...
    assert peri_scribe.fire_differential.polygonal_area(collection) is None
...
def test_geometry_difference_returns_current_without_previous() -> None:
    geometry = square(1.0)
    result = peri_scribe.fire_differential.geometry_difference(
        geometry,
        None,
    )
    assert result is not None
...
def test_geometry_difference_returns_none_for_missing_current() -> None:
    assert peri_scribe.fire_differential.geometry_difference(None, square(1.0)) is None
...
def test_geometry_intersection_returns_none_for_missing() -> None:
    assert (
        peri_scribe.fire_differential.geometry_intersection(None, square(1.0)) is None
    )
    assert (
        peri_scribe.fire_differential.geometry_intersection(square(1.0), None) is None
    )


def test_geometry_intersection_keeps_shared_area() -> None:
    result = peri_scribe.fire_differential.geometry_intersection(
        square(2.0),
        square(1.5),
    )
    assert result is not None
...
def test_corrected_geometries_removes_later_reductions() -> None:
    ...
    corrected = peri_scribe.fire_differential.corrected_geometries(geometries)
    expected = square(1.0)
    assert all(
...
def test_corrected_geometries_handles_empty() -> None:
    assert peri_scribe.fire_differential.corrected_geometries([]) == []
...
def test_growth_difference_subtracts_most_recent_present() -> None:
    assert peri_scribe.fire_differential.growth_difference(
        150.0,
        [None, 100.0],
    ) == pytest.approx(50.0)
...
def test_growth_difference_returns_none_when_current_missing() -> None:
    assert peri_scribe.fire_differential.growth_difference(None, [100.0]) is None
    assert (
        peri_scribe.fire_differential.growth_difference(float("nan"), [100.0]) is None
    )
...
def test_identity_key_normalizes_missing() -> None:
    ...
    assert peri_scribe.fire_differential.identity_key(
        row,
...
def test_fire_positions_groups_by_identity() -> None:
    ...
    assert peri_scribe.fire_differential.fire_positions(frame) == [[0, 1], [2]]
...
def test_differential_perimeter_dataframe_builds_growth_rows() -> None:
    ...
    output = peri_scribe.fire_differential.differential_perimeter_dataframe(frame)
    assert (
        list(output.columns)
        == peri_scribe.fire_differential.DIFFERENTIAL_PERIMETER_COLUMNS
    )
...
def test_write_history_of_differential_geography_writes_two_layers(
    ...
    result = peri_scribe.fire_differential.write_history_of_differential_geography(
        pathlib.Path("data/2026"),
    )
