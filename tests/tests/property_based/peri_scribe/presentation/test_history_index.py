...
import tests.helpers.strategies.peri_scribe.presentation.history_index


@hypothesis.given(
    rows=tests.helpers.strategies.peri_scribe.presentation.history_index.history_rows(),
...
def test_history_row_index_positions_for_matches_dataframe_filtering(
    ...
    expected = tests.helpers.reference.peri_scribe.presentation.history_index.select(
        frame,
        identifiers,
        name,
    )
