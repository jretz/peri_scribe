...
import tests.helpers.strategies.peri_scribe.kml.geometry
...
def test_perimeter_geometry_preserves_parts_holes_and_rounded_coordinates(
    ...
    for actual, original in zip(polygons, expected, strict=True):
        rings = tests.helpers.peri_scribe.kml.geometry.coordinate_rings(actual)
...
@hypothesis.given(
    before=tests.helpers.strategies.peri_scribe.kml.geometry.xml_text(),
    content=tests.helpers.strategies.peri_scribe.kml.geometry.xml_text(),
    after=tests.helpers.strategies.peri_scribe.kml.geometry.xml_text(),
)
