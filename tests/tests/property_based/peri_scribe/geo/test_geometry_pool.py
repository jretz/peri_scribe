...
import tests.helpers.strategies.peri_scribe.geo.geometry_pool


@hypothesis.given(
    observations=tests.helpers.strategies.peri_scribe.geo.geometry_pool.observations(),
)
def test_shared_geometry_matches_reference_cache(
    ...
    for digest, wkb in observations:
        state, geometry = peri_scribe.geo.geometry_pool.shared_geometry(
            state,
            digest,
            wkb,
        )
...
def test_shared_geometry_preserves_generated_snapshots(
    ...
    for snapshot, expected in snapshots:
        for digest, wkb in dict.fromkeys(observations):
            updated, geometry = peri_scribe.geo.geometry_pool.shared_geometry(
                snapshot,
                digest,
                wkb,
            )
