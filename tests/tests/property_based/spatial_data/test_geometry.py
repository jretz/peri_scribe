...
def test_polygonal_parts_preserves_polygons_through_nested_collections(
    ...
    assert spatial_data.geometry.polygonal_parts(geometry) == expected
...
def test_transform_coordinates_round_trips_projection(
    ...
    projected = spatial_data.geometry.transform_coordinates(
        geometry,
        pyproj.Transformer.from_crs(4326, 3857, always_xy=True),
    )
    restored = spatial_data.geometry.transform_coordinates(
        projected,
