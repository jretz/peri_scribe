...
def test_read_layer_chunks_preserves_sparse_features_in_bounded_chunks(
    ...
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        tests.helpers.factories.spatial_data.layers.write_sparse_layer(
            path,
            identifiers,
        )
        expected = geopandas.read_file(path, layer="features")
        chunks = list(
            spatial_data.layers.read_layer_chunks(path, "features", chunk_size),
        )
