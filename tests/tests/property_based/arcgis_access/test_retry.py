...
import tests.helpers.strategies.arcgis_access.retry
...
@hypothesis.given(
    representations=tests.helpers.strategies.arcgis_access.retry.rate_limit_representations(),
)
def test_rate_limit_retry_agrees_for_payload_and_json_text(
    ...
    assert arcgis_access.retry.rate_limit_retry(ValueError(text)) == (
        arcgis_access.retry.rate_limit_retry(ValueError(payload))
    )
...
@hypothesis.given(
    outcomes=hypothesis.strategies.lists(
        hypothesis.strategies.from_type(
            tests.helpers.factories.arcgis_access.retry.AttemptOutcome,
        ),
        max_size=10,
    ),
...
def test_run_with_retry_stops_at_success_fatal_error_or_retry_limit(
    outcomes: list[tests.helpers.factories.arcgis_access.retry.AttemptOutcome],
    maximum_retries: int,
) -> None:
    result = object()
    effects = tests.helpers.factories.arcgis_access.retry.query_effects(
        outcomes,
        result,
    )
    terminal_outcomes = {
        tests.helpers.factories.arcgis_access.retry.AttemptOutcome.SUCCESS,
        tests.helpers.factories.arcgis_access.retry.AttemptOutcome.FATAL_ERROR,
    }
    first_terminal = next(
        (
            index
    ...
    with unittest.mock.patch("time.sleep") as sleep:
        if isinstance(expected, Exception):
            with pytest.raises(type(expected)) as raised:
                arcgis_access.retry.run_with_retry(
                    "generated-feed",
                    query,
