...
def test_update_from_row_dates_perimeter_attributes_by_incident_time() -> None:
    row = pd.Series({
        "observation_time": tests.helpers.factories.peri_scribe.incidents.time(2),
        "source_attributes": {
            "attr_ModifiedOnDateTime_dt": (
                tests.helpers.factories.peri_scribe.incidents
            ).time(
                4,
            ),
    ...
    assert (
        result.observation_time == tests.helpers.factories.peri_scribe.incidents.time(4)
    )
...
def test_update_from_row_preserves_zero_and_skips_invalid_values() -> None:
    result = peri_scribe.incidents.update_from_row(
        pd.Series({
            "observation_time": tests.helpers.factories.peri_scribe.incidents.time(1),
            "percent_contained": 0,
            "incident_size": -1,
...
def test_reconcile_updates_prefers_direct_report_and_fills_missing_fields() -> None:
    direct = tests.helpers.factories.peri_scribe.incidents.update(1, 100)
...
def test_reconcile_updates_preserves_confirmation_for_matching_value() -> None:
    perimeter = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(1, 100, confirmed=True),
        source="wfigs_perimeter",
    )
    direct = tests.helpers.factories.peri_scribe.incidents.update(1, 100)
...
def test_reconcile_updates_does_not_confirm_a_conflicting_value() -> None:
    ...
    direct = tests.helpers.factories.peri_scribe.incidents.update(1, 200)
    result = peri_scribe.incidents.reconcile_updates([perimeter, direct])
...
def test_reconcile_updates_retains_newest_confirmation_for_matching_value() -> None:
    perimeter = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(2, 100, confirmed=True),
        source="wfigs_perimeter",
    )
    direct = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(2, 100, confirmed=True),
        report_time=tests.helpers.factories.peri_scribe.incidents.time(1),
    )
...
def test_update_from_row_does_not_invent_incident_size_from_perimeter_columns() -> None:
    result = peri_scribe.incidents.update_from_row(
        pd.Series({
            "modified_time": tests.helpers.factories.peri_scribe.incidents.time(1),
            "incident_size": 500,
            "area_acres": 500,
...
def test_reconcile_updates_ignores_dispatch_reversion_to_pre_report_values() -> None:
    report = tests.helpers.factories.peri_scribe.incidents.update(
        1,
        39073,
        confirmed=True,
    )
    dispatch = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(2, 4000),
        report_time=report.report_time,
    )
    correction = tests.helpers.factories.peri_scribe.incidents.update(
        3,
        39103,
        confirmed=True,
    )
...
def test_reconcile_updates_accepts_newer_reporting_evidence() -> None:
    first = tests.helpers.factories.peri_scribe.incidents.update(1, 100, confirmed=True)
    later = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(3, 110),
        report_time=tests.helpers.factories.peri_scribe.incidents.time(2),
    )
...
def test_reconcile_updates_accepts_dispatch_growth_between_formal_reports() -> None:
    ...
    dispatch = dataclasses.replace(
        tests.helpers.factories.peri_scribe.incidents.update(2, 3171),
        report_time=report.report_time,
    )
...
def test_reconcile_updates_skips_empty_updates() -> None:
    assert (
        peri_scribe.incidents.reconcile_updates([
            dataclasses.replace(
                tests.helpers.factories.peri_scribe.incidents.update(1, 100),
                measurements={},
            ),
        ])
        == ()
...
def test_history_reads_normalized_records_and_preserves_confirmation() -> None:
    ...
    assert result[0].report_time == tests.helpers.factories.peri_scribe.incidents.time(
        1,
    )
