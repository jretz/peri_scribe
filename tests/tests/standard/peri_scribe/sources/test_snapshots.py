...
import tests.helpers.doubles.peri_scribe.sources.snapshots
...
def test_source_file_relative_path_places_file_in_bucket_directory() -> None:
    assert tests.helpers.factories.peri_scribe.sources.snapshots.source_file(
        serial_number=2037,
...
def test_source_file_from_path_parses_serial_and_timestamp() -> None:
    ...
    ) == tests.helpers.factories.peri_scribe.sources.snapshots.source_file(
        serial_number=2037,
        last_edit_timestamp=1787118540625,
    )
...
def test_source_file_from_path_rejects_missing_timestamp() -> None:
    with pytest.raises(ValueError, match="Unrecognized snapshot filename"):
        peri_scribe.sources.snapshots.SourceFile.from_path(pathlib.Path("000001.gpkg"))


def test_next_serial_number_starts_at_zero_without_existing_files() -> None:
    assert peri_scribe.sources.snapshots.next_serial_number([], 123) == 0
...
def test_existing_source_files_returns_empty_without_directory(
    ...
    assert (
        peri_scribe.sources.snapshots.existing_source_files(pathlib.Path("/missing"))
        == []
    )
...
def test_existing_source_files_returns_source_files_sorted_by_serial(
    ...
    tests.helpers.doubles.peri_scribe.sources.snapshots.stub_directory(
        monkeypatch,
        files,
    )
...
def test_geo_package_files_returns_empty_list_without_directory(
    ...
    monkeypatch.setattr(pathlib.Path, "is_dir", lambda _self: False)
    assert (
        peri_scribe.sources.snapshots.geo_package_files(pathlib.Path("/missing")) == []
    )
...
def test_geo_package_files_skips_non_snapshot_files(
    ...
    assert peri_scribe.sources.snapshots.geo_package_files(directory) == [keep]
...
def test_geo_package_files_raises_system_exit_when_tree_cannot_be_read(
    ...
    fake_rglob = (
        tests.helpers.doubles.peri_scribe.sources.snapshots.deny_snapshot_listing
    )
    ...
    with pytest.raises(
        ...
        peri_scribe.sources.snapshots.geo_package_files(directory)
...
def test_source_directory_path_places_source_under_sources() -> None:
    assert peri_scribe.sources.snapshots.source_directory_path(
        pathlib.Path("/base"),
        2026,
        "Feed_0",
...
def test_source_name_from_snapshot_path_returns_feed_directory_name() -> None:
    assert (
        peri_scribe.sources.snapshots.source_name_from_snapshot_path(
...
def test_validation_directory_path_places_validation_under_year() -> None:
    assert peri_scribe.sources.snapshots.validation_directory_path(
        pathlib.Path("/data/2026"),
    ) == pathlib.Path("/data/2026/validation")


def test_validation_geopackage_path_places_file_in_validation_directory() -> None:
    assert peri_scribe.sources.snapshots.validation_geopackage_path(
        pathlib.Path("/data/2026"),
        "CA_Perimeters_NIFC_FIRIS_public_view_0",
    ) == pathlib.Path(
...
def test_base_directory_for_year_directory_returns_parent_of_data_directory() -> None:
    assert peri_scribe.sources.snapshots.base_directory_for_year_directory(
        pathlib.Path("/base/data/2026"),
    ) == pathlib.Path("/base")


def test_fire_index_path_places_index_in_sources_directory() -> None:
    assert peri_scribe.sources.snapshots.fire_index_path(
        pathlib.Path("/data/2026"),
