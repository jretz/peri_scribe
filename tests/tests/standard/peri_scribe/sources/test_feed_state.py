    ...
    monkeypatch.setattr(
        ...
        lambda _path, _feed: (
            tests.helpers.factories.peri_scribe.sources.changes.change_dataframe([
                tests.helpers.factories.peri_scribe.sources.changes.SAMPLE_FEATURE_ROW,
...
def test_stored_status_object_ids_returns_empty_without_object_id_column() -> None:
    feed = tests.helpers.factories.peri_scribe.sources.feed_types.change_feed()
    existing = tests.helpers.factories.peri_scribe.sources.changes.status_dataframe([
        (1, "Active", (0.0, 0.0)),
    ]).drop(columns=["OBJECTID"])
    assert (
        peri_scribe.sources.feed_state.stored_status_object_ids(
...
def test_write_current_state_ignores_missing_old_state_file(
    ...
    flaky_unlink = (
        tests.helpers.doubles.peri_scribe.sources.changes
    ).make_disappearing_state_unlink(
