...
import tests.helpers.doubles.peri_scribe.sources.validation
...
def test_validate_feed_treats_stored_frame_without_object_id_as_missing() -> None:
    ...
    result = peri_scribe.sources.validation.validate_feed(
        tests.helpers.factories.peri_scribe.sources.feed_types.change_feed(),
        complete,
        tests.helpers.factories.peri_scribe.sources.validation.frame_without_object_id(),
    )
    assert result.has_problems
...
def test_validate_complete_sources_validates_each_feed_in_order(
    ...
    feeds = [
        tests.helpers.factories.peri_scribe.sources.validation.validation_feed(0),
        tests.helpers.factories.peri_scribe.sources.validation.validation_feed(1),
    ]
    complete_frames = {
        "Fires0_0": (
            tests.helpers.factories.peri_scribe.sources.changes
        ).change_dataframe([
            (1, "a", (0.0, 0.0)),
        ]),
        "Fires1_0": (
    ...
    read_layer_dataframe = (
        tests.helpers.doubles.peri_scribe.sources.validation
    ).make_complete_snapshot_reader(
    ...
    assert read_calls == [
        (year_directory / "validation" / "Fires0_0.gpkg", feeds[0]),
        (year_directory / "validation" / "Fires1_0.gpkg", feeds[1]),
    ]
