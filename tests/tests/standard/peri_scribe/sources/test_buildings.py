...
def test_tile_id_puts_boundary_points_in_upper_tile() -> None:
    ...
    assert (
        peri_scribe.sources.buildings.encode_longitude(0.5)
        == tests.helpers.factories.peri_scribe.sources.buildings.QUANTIZED_HALF_DEGREE
    )
    assert (
...
def test_decode_payload_round_trips_encoded_points(tmp_path: pathlib.Path) -> None:
    points = np.asarray([[0.2, 0.2], [0.3, 0.3], [100.5, 40.25]], dtype=float)
    path = tmp_path / "buildings.sqlite"
    tests.helpers.factories.peri_scribe.sources.buildings.write_database(points, path)
    connection = sqlite3.connect(path)
    try:
        rows = connection.execute(
...
def test_building_counts_within_reads_shared_tile_once(
    ...
    read = tests.helpers.doubles.peri_scribe.sources.buildings.make_tile_read_recorder(
