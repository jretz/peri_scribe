...
import tests.helpers.factories.peri_scribe.fires.differential
...
def test_polygonal_area_extracts_polygons_from_geometry_collection() -> None:
    ...
    assert result.equals(tests.helpers.factories.geometry.square(1.0))
...
def test_geometry_difference_subtracts_overlap() -> None:
    ...
    assert result.equals(
        tests.helpers.factories.geometry.square(2.0).difference(
            tests.helpers.factories.geometry.square(1.0),
        ),
    )
...
def test_geometry_intersection_keeps_shared_area() -> None:
    result = peri_scribe.fires.differential.geometry_intersection(
        tests.helpers.factories.geometry.square(2.0),
        tests.helpers.factories.geometry.square(1.5),
    )
    assert result is not None
    assert result.equals(tests.helpers.factories.geometry.square(1.5))
...
def test_corrected_geometries_removes_later_reductions() -> None:
    ...
    expected = tests.helpers.factories.geometry.square(1.0)
    assert all(
...
def test_differential_perimeter_dataframe_builds_growth_rows() -> None:
    ...
    frame = tests.helpers.factories.peri_scribe.fires.differential.full_perimeter_frame(
        records,
        [
            tests.helpers.factories.geometry.square(1.0),
            tests.helpers.factories.geometry.square(2.0),
    ...
    assert output.geometry.iloc[0].equals(tests.helpers.factories.geometry.square(1.0))
    assert output.geometry.iloc[1].equals(
        tests.helpers.factories.geometry.square(1.5).difference(
            tests.helpers.factories.geometry.square(1.0),
        ),
    )
...
def test_differential_perimeter_dataframe_back_propagates_null_values() -> None:
    ...
    frame = tests.helpers.factories.peri_scribe.fires.differential.full_perimeter_frame(
        ...
        [
            tests.helpers.factories.geometry.square(1.0),
            tests.helpers.factories.geometry.square(0.5),
            tests.helpers.factories.geometry.square(1.5),
        ],
    )
    output = peri_scribe.fires.differential.differential_perimeter_dataframe(frame)
...
def test_differential_perimeter_dataframe_skips_collapsed_growth(
    ...
    collapsing_difference = (
        tests.helpers.doubles.peri_scribe.fires.differential.make_collapsing_difference(
...
def test_differential_perimeter_dataframe_parallel_matches_single_worker(
    ...
    frame, fire_identifiers = (
        tests.helpers.factories.peri_scribe.fires.differential.multiple_fire_perimeter_frame()
    )
...
def test_write_history_of_differential_geography_writes_two_layers(
    ...
    perimeters = (
        tests.helpers.factories.peri_scribe.fires.differential.full_perimeter_frame(
            [],
            [],
        )
    )
