...
def test_area_groups_keeps_unidentified_fires_separate_by_name() -> None:
    ...
    groups = peri_scribe.presentation.selection.area_groups(
        frame,
        {"alias": "canonical"},
    )
...
def test_fires_with_qualifying_area_includes_geometry_without_reported_acres(
    ...
    assert peri_scribe.presentation.selection.fires_with_qualifying_area(
        mapped_fire,
...
def test_fires_with_qualifying_area_does_not_split_alias_evidence(
    ...
    assert not peri_scribe.presentation.selection.fires_with_qualifying_area(
        mapped_fire,
        points,
        25 * units.acres,
        aliases={"alias": "example"},
...
def test_identifiers_includes_identifier_and_aliases() -> None:
    ...
    assert peri_scribe.presentation.selection.identifiers(entry) == {
        "2026-casnd-150541",
        "2026-casnd-26150541",
        "guid",
    }
...
def test_identifiers_omits_none_identifier() -> None:
    ...
    assert peri_scribe.presentation.selection.identifiers(entry) == frozenset()
...
def test_point_locations_keep_last_point_per_fire() -> None:
    ...
    by_identifier, by_name = peri_scribe.presentation.selection.point_locations(points)
...
def test_perimeter_groups_omits_missing_and_empty_geometry(
    ...
    assert peri_scribe.presentation.selection.perimeter_groups(perimeters) == ({}, {})


def test_fire_point_matches_identifier() -> None:
    point = shapely.geometry.Point(1.0, 1.0)
    result = peri_scribe.presentation.selection.fire_point(
        frozenset({"id-a"}),
        "Bug",
        {"id-a": point},
        {},
...
def test_fire_point_returns_none_when_name_missing() -> None:
    assert (
        peri_scribe.presentation.selection.fire_point(frozenset(), "Bug", {}, {})
        is None
    )
...
def test_fire_point_returns_none_when_identifier_missing() -> None:
    assert (
        peri_scribe.presentation.selection.fire_point(
            frozenset({"id-a"}),
            "Bug",
            {},
            {},
...
def test_fire_area_key_keys_by_identifier() -> None:
    assert peri_scribe.presentation.selection.fire_area_key("id-bug", "Bug") == (
        "id",
        "id-bug",
    )
...
def test_fire_area_key_keys_by_name_when_identifier_missing() -> None:
    assert peri_scribe.presentation.selection.fire_area_key(None, "Bug") == (
        "name",
        "Bug",
    )
...
def test_fires_with_qualifying_area_keeps_fire_at_minimum() -> None:
    perimeters = tests.helpers.factories.peri_scribe.presentation.fire_data.area_frame(
        "area_acres",
        [("id-bug", "Bug")],
        [peri_scribe.presentation.selection.MINIMUM_FIRE_AREA.m_as("acres")],
    )
...
def test_fires_with_qualifying_area_excludes_fire_below_minimum() -> None:
    ...
    assert (
        peri_scribe.presentation.selection.fires_with_qualifying_area(
            perimeters,
            points,
            peri_scribe.presentation.selection.MINIMUM_FIRE_AREA,
        )
...
def test_fire_qualifies_matches_any_identifier() -> None:
    assert peri_scribe.presentation.selection.fire_qualifies(
...
def test_fire_qualifies_rejects_unmatched_fire() -> None:
    assert not peri_scribe.presentation.selection.fire_qualifies(
        frozenset({"id-bug"}),
        "Bug",
...
def test_fire_point_location_uses_known_point() -> None:
    point = shapely.geometry.Point(1.0, 1.0)
    result = peri_scribe.presentation.selection.fire_point_location(
        frozenset({"id-a"}),
        "Bug",
        {"id-a": point},
        {},
...
def test_fire_point_location_returns_none_without_geometry() -> None:
    assert (
        peri_scribe.presentation.selection.fire_point_location(
            frozenset({"id-a"}),
            "Bug",
            {},
            {},
...
def test_perimeter_groups_carries_shared_measurements() -> None:
    ...
    expected_area = spatial_data.measurements.area(shape)
    ...
    by_identifier, _by_name = peri_scribe.presentation.selection.perimeter_groups(frame)
