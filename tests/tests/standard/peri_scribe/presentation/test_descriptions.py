"""Tests for peri_scribe.presentation.descriptions."""
...
def test_format_number_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_number(None) is None


def test_format_number_rounds_to_whole_number() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_number(1234567.89) == "1,234,568"
    )


def test_format_number_keeps_decimal_places_and_separators() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_number(1234567.89, 2)
        == "1,234,567.89"
    )


def test_format_number_drops_trailing_zeros() -> None:
    assert peri_scribe.presentation.descriptions.format_number(6.0, 1) == "6"


def test_format_area_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_area(None) is None


def test_format_area_uses_whole_acres_for_large_fires() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_area(102003.46 * units.acres)
        == "102,003 acres"
    )
...
def test_format_area_uses_one_decimal_for_small_fires() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_area(6.5 * units.acres)
        == "6.5 acres"
    )
...
def test_format_area_uses_two_decimals_for_fractional_acres() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_area(0.017 * units.acres)
        == "0.02 acres"
    )


def test_format_percent_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_percent(None) is None


def test_format_percent_uses_whole_percent() -> None:
    assert peri_scribe.presentation.descriptions.format_percent(77.0) == "77%"


def test_format_percent_uses_one_decimal_for_fractional_percent() -> None:
    assert peri_scribe.presentation.descriptions.format_percent(0.5) == "0.5%"


def test_format_miles_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_miles(None) is None


def test_format_miles_adds_unit_and_one_decimal() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_miles(33.1 * units.miles)
        == "33.1 miles"
    )
...
def test_format_miles_drops_trailing_zero() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_miles(33.0 * units.miles)
        == "33 miles"
    )


def test_format_perimeter_length_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_perimeter_length(None) is None


def test_round_to_significant_digits_returns_zero_for_zero() -> None:
    assert peri_scribe.presentation.descriptions.round_to_significant_digits(
        0.0,
        3,
    ) == pytest.approx(0.0)
...
def test_format_perimeter_length_keeps_one_decimal_for_small_lengths() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_perimeter_length(
            0.1499 * units.miles,
        )
        == "0.1"
    )
...
def test_format_miles_caps_large_lengths_to_three_significant_digits() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_miles(123.6 * units.miles)
        == "124 miles"
    )
    assert (
        peri_scribe.presentation.descriptions.format_miles(5678.123 * units.miles)
        == "5,680 miles"
    )
...
def test_format_miles_keeps_small_lengths_at_one_decimal() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_miles(0.1499 * units.miles)
        == "0.1 miles"
    )
    assert (
        peri_scribe.presentation.descriptions.format_miles(math.pi * units.miles)
        == "3.1 miles"
    )
...
def test_format_containment_returns_none_without_percent() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_containment(
            None,
            33.1 * units.miles,
        )
        is None
...
def test_format_containment_uses_bare_percent_without_length() -> None:
    assert peri_scribe.presentation.descriptions.format_containment(68.0, None) == "68%"
...
def test_format_cost_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_cost(None) is None


def test_format_cost_adds_dollar_sign_and_separators() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_cost(104_600_000.0 * units.dollars)
        == "$104,600,000"
    )


def test_format_personnel_count_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_personnel_count(None) is None


def test_format_personnel_count_uses_whole_numbers_with_separators() -> None:
    assert (
        peri_scribe.presentation.descriptions.format_personnel_count(1234.0) == "1,234"
    )


def test_format_pacific_time_returns_none_for_none() -> None:
    assert peri_scribe.presentation.descriptions.format_pacific_time(None) is None


def test_format_pacific_time_marks_pacific_daylight_time() -> None:
    value = datetime.datetime(2026, 8, 5, 20, 30, tzinfo=datetime.UTC)
    assert (
        peri_scribe.presentation.descriptions.format_pacific_time(value)
        == "08/05 13:30 PDT"
    )
...
def test_description_rows_includes_every_present_value() -> None:
    assert peri_scribe.presentation.descriptions.description_rows(
...
def test_description_rows_returns_none_for_missing_values() -> None:
    ...
    assert peri_scribe.presentation.descriptions.description_rows(description) == [
        ("Area", None),
