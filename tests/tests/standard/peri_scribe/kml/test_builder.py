...
import tests.helpers.factories.peri_scribe.kml.builder
...
def test_fire_kml_names_the_document() -> None:
    ...
    assert (
        document.findtext(tests.helpers.peri_scribe.kml.parsing.kml_tag("name"))
        == "PeriScribe Fires 2026"
    )
    attribution = document.findtext(
        tests.helpers.peri_scribe.kml.parsing.kml_tag("description"),
    )
    ...
    assert (
        tests.helpers.peri_scribe.kml.parsing.folder_list_item_type(top_level)
        == "radioFolder"
    )
...
def test_fire_kml_puts_top_fires_before_status_folders() -> None:
    ...
    top_level = tests.helpers.peri_scribe.kml.parsing.folder_named(document, "test")
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(top_level) == [
        "Top Fires by Name",
        "Top Fires by Score",
        "Active Fires",
    ]
    top_by_name = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Top Fires by Name",
    )
    top_by_score = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Top Fires by Score",
    )
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(top_by_name) == [
        "Alpha",
        "Zulu",
    ]
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(top_by_score) == [
        "Zulu",
        "Alpha",
    ]
...
def test_fire_kml_puts_new_folders_before_top_fires() -> None:
    ...
    with time_machine.travel(
        tests.helpers.factories.peri_scribe.kml.builder.SCENARIO_TIME,
    ):
        document = tests.helpers.peri_scribe.kml.parsing.document_from(
            peri_scribe.kml.builder.fire_kml(fires, "test", scores),
        )
...
def test_fire_kml_loads_new_folders_unchecked() -> None:
    ...
    top_level = tests.helpers.peri_scribe.kml.parsing.folder_named(document, "test")
    by_name = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Top Fires by Name",
    )
    active = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Active Fires",
    )
    ...
    for name in tests.helpers.factories.peri_scribe.kml.builder.NEW_FOLDER_NAMES:
        folder = tests.helpers.peri_scribe.kml.parsing.folder_named(top_level, name)
        ...
        assert tests.helpers.peri_scribe.kml.parsing.folder_names(folder) == ["Alpha"]
    assert tests.helpers.peri_scribe.kml.parsing.visibility(by_name) is None
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_visible(by_name)
    assert tests.helpers.peri_scribe.kml.parsing.visibility(active) == 0
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_invisible(active)
...
def test_fire_kml_lists_type_one_fires_by_name() -> None:
    ...
    top_level = tests.helpers.peri_scribe.kml.parsing.folder_named(document, "test")
    type_one = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Type 1 Fires",
    )
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(type_one) == [
        "Alpha",
        "Zulu",
    ]
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_invisible(type_one)
...
def test_fire_kml_loads_top_fires_by_name_checked() -> None:
    ...
    by_score = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Top Fires by Score",
    )
    active = tests.helpers.peri_scribe.kml.parsing.folder_named(
    ...
    assert tests.helpers.peri_scribe.kml.parsing.visibility(by_name) is None
    assert tests.helpers.peri_scribe.kml.parsing.visibility(by_score) == 0
    assert tests.helpers.peri_scribe.kml.parsing.visibility(active) == 0
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_visible(by_name)
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_invisible(active)
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_invisible(by_score)
...
def test_fire_kml_checks_active_fires_without_top_fires() -> None:
    ...
    assert tests.helpers.peri_scribe.kml.parsing.visibility(active) is None
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_visible(active)
...
def test_fire_kml_checks_inactive_fires_without_top_or_active_fires() -> None:
    ...
    inactive = tests.helpers.peri_scribe.kml.parsing.folder_named(
        top_level,
        "Inactive Fires",
    )
    assert tests.helpers.peri_scribe.kml.parsing.visibility(inactive) is None
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_visible(inactive)


def test_fire_kml_without_fires_has_no_top_level_folders() -> None:
    empty_scores = peri_scribe.models.FireScores(version="test", fires=[])
    documents = [
        tests.helpers.peri_scribe.kml.parsing.document_from(
...
def test_fire_kml_holds_fires_directly_under_status_folders() -> None:
    ...
    perimeters = tests.helpers.factories.peri_scribe.kml.parsing.geometry_frame(
        [
            ("id-bug", "Bug", tests.helpers.factories.geometry.square(1.0)),
            ("id-alta", "ALTA", tests.helpers.factories.geometry.square(2.0)),
        ],
    ...
    assert tests.helpers.peri_scribe.kml.parsing.folder_list_item_type(active) is None
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(active) == ["Bug"]
    bug_folder = tests.helpers.peri_scribe.kml.parsing.folder_named(active, "Bug")
    assert (
        tests.helpers.peri_scribe.kml.parsing.folder_list_item_type(bug_folder) is None
    )
    assert tests.helpers.peri_scribe.kml.parsing.placemark_names(bug_folder) == [
        "Bug",
    ...
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(inactive) == ["ALTA"]
    alta_folder = tests.helpers.peri_scribe.kml.parsing.folder_named(inactive, "ALTA")
    assert tests.helpers.peri_scribe.kml.parsing.placemark_names(alta_folder) == [
        "ALTA",
        "08/15 13:00 Perimeter",
    ]
    assert tests.helpers.peri_scribe.kml.parsing.folder_names(alta_folder) == [
        "Interior",
    ]
...
def test_fire_kml_builds_active_and_inactive_folders() -> None:
    ...
    perimeters = tests.helpers.factories.peri_scribe.kml.parsing.geometry_frame(
        [
            ...
            ("id-bug", "Bug", tests.helpers.factories.geometry.square(3.0)),
        ],
        observation_times=[
    ...
    assert tests.helpers.peri_scribe.kml.parsing.placemark_names(
        tests.helpers.peri_scribe.kml.parsing.folder_named(bug_folder, "Perimeters"),
    ...
    assert tests.helpers.peri_scribe.kml.parsing.folder_list_item_type(inactive) is None
    alta_folder = tests.helpers.peri_scribe.kml.parsing.folder_named(inactive, "ALTA")
    assert tests.helpers.peri_scribe.kml.parsing.placemark_names(alta_folder) == [
        "ALTA",
    ]
    assert (
        tests.helpers.peri_scribe.kml.parsing.draw_order(
            tests.helpers.peri_scribe.kml.parsing.placemark_named(alta_folder, "ALTA"),
        )
        == 1
    )
    ...
    style_ids = {
        ...
        if child.tag == tests.helpers.peri_scribe.kml.parsing.kml_tag("Style")
    }
...
def test_fire_kml_hides_inactive_fires_tree() -> None:
    ...
    tests.helpers.assertions.peri_scribe.kml.parsing.assert_tree_invisible(inactive)
    ...
    bug_folder = tests.helpers.peri_scribe.kml.parsing.folder_named(active, "Bug")
    assert tests.helpers.peri_scribe.kml.parsing.visibility(active) is None
    assert tests.helpers.peri_scribe.kml.parsing.visibility(bug_folder) is None
    for placemark in bug_folder.findall(
        tests.helpers.peri_scribe.kml.parsing.kml_tag("Placemark"),
    ):
        assert tests.helpers.peri_scribe.kml.parsing.visibility(placemark) is None
    assert (
        tests.helpers.peri_scribe.kml.parsing.visibility(
            tests.helpers.peri_scribe.kml.parsing.tour_named(bug_folder, "Progression"),
        )
        is None
    )
...
def test_fire_kml_shows_derived_point_for_inactive_fire_without_location() -> None:
    ...
    assert tests.helpers.peri_scribe.kml.parsing.placemark_names(
        tests.helpers.peri_scribe.kml.parsing.folder_named(alta_folder, "Interior"),
    ) == ["Interior"]
    assert (
...
def test_write_archive_writes_compressed_document(
    ...
    archives: list[tests.helpers.doubles.peri_scribe.kml.builder.FakeArchive] = []

    monkeypatch.setattr(
        zipfile,
        "ZipFile",
        tests.helpers.doubles.peri_scribe.kml.builder.recording_archive_factory(
            archives,
        ),
    )
...
def test_create_kmz_reads_history_and_writes_kmz(
    ...
    read_layer = (
        tests.helpers.doubles.peri_scribe.kml.builder.make_history_layer_reader(
...
def test_create_kmz_excludes_fires_without_qualifying_area(
    ...
    perimeters = tests.helpers.factories.peri_scribe.kml.parsing.geometry_frame(
        [
            ("id-bug", "Bug", tests.helpers.factories.geometry.square(0.01)),
            ("id-tiny", "Tiny", tests.helpers.factories.geometry.square(0.001)),
        ],
