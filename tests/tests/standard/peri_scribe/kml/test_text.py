...
def test_fire_description_uses_latest_incident_values() -> None:
    ...
    description = peri_scribe.kml.text.fire_description(
        entry,
        tests.helpers.factories.peri_scribe.kml.fire_data.description_perimeter_frame(),
        tests.helpers.factories.peri_scribe.kml.fire_data.description_point_frame(),
    ...
    assert description.area.m_as("acres") == pytest.approx(
        peri_scribe.units.area(tests.helpers.factories.geometry.square(2.0)).m_as(
            "acres",
        ),
    )
