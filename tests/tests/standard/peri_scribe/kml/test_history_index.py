...
import tests.helpers.assertions.peri_scribe.kml.history_index
...
def test_from_frame_indexes_rows_by_identifier_and_name() -> None:
    frame = tests.helpers.factories.peri_scribe.kml.history_index.history_frame([
        ("id-bug", "Bug"),
        ("id-alta", "ALTA"),
        ("id-bug", "Bug"),
        (None, "Bug"),
...
def test_positions_for_matches_reference_filters() -> None:
    frames = [
        tests.helpers.factories.peri_scribe.kml.history_index.history_frame([
            ("id-bug", "Bug"),
            ("id-alta", "ALTA"),
            ("id-bug", "Bug"),
            ("id-bug", "Bug"),
    ...
    for frame in frames:
        ...
        for fire_identifiers, name in probes:
            expected = (
                tests.helpers.reference.peri_scribe.kml.history_index
            ).reference_selection(
                frame,
                fire_identifiers,
                name,
            ...
            tests.helpers.assertions.peri_scribe.kml.history_index.assert_same_rows(
                actual,
                expected,
            )
