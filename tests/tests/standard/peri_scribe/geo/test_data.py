...
import tests.helpers.doubles.peri_scribe.geo.data
...
def test_query_with_retry_succeeds_on_first_attempt(
    ...
    layer = tests.helpers.doubles.peri_scribe.geo.data.QueryStub(outcomes)
...
def test_query_with_retry_exhausts_retries_and_raises(
    ...
    with pytest.raises(
        ValueError,
        match=re.escape(
            str(tests.helpers.factories.peri_scribe.retry.RATE_LIMIT_ERROR_PAYLOAD),
        ),
    ):
...
def test_query_with_retry_logs_rate_limit_reason(
    ...
    assert captured[0]["retry_delay"] == {
        "value": tests.helpers.factories.peri_scribe.retry.RATE_LIMIT_RETRY_AFTER.m_as(
            "seconds",
        ),
        "units": units.seconds,
    }
...
def test_query_object_ids_with_retry_returns_object_ids() -> None:
    layer = tests.helpers.doubles.peri_scribe.geo.data.IdQueryStub({
        "objectIds": [3, 4],
    })
...
def test_query_object_ids_with_retry_raises_without_object_ids() -> None:
    layer = tests.helpers.doubles.peri_scribe.geo.data.IdQueryStub({"count": 0})
