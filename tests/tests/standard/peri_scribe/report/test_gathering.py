...
def test_located_entries_keeps_a_fire_from_a_single_use_iterator(
    ...
    fire = tests.helpers.factories.peri_scribe.report.gathering.make_fire(
        "Bug",
        "id-bug",
    )
...
def test_report_details_distinguishes_a_name_from_a_matching_identifier() -> None:
    unidentified = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "2026-a",
    )
    identified = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "2026-a",
        identifier="2026-a",
    )
...
def test_fire_locations_distinguishes_a_name_from_a_matching_identifier(
    ...
    identified = tests.helpers.factories.peri_scribe.report.gathering.make_fire(
        "River",
        "2026-a",
    )
    monkeypatch.setattr(
...
def test_report_details_returns_each_fire_once_sorted_by_name() -> None:
    bug = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "Bug",
        identifier="id-bug",
    )
    fire = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "Fire",
        identifier="id-fire",
    )
...
def test_report_details_sorts_case_insensitively() -> None:
    alpha = tests.helpers.factories.peri_scribe.report.gathering.make_entry("alpha")
    beta = tests.helpers.factories.peri_scribe.report.gathering.make_entry("Beta")
...
def test_report_details_keeps_same_name_fires_distinct() -> None:
    first = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "Bug",
        identifier="id-first",
    )
    second = tests.helpers.factories.peri_scribe.report.gathering.make_entry(
        "Bug",
...
def test_report_details_identifies_unnamed_fire_by_name() -> None:
    bug = tests.helpers.factories.peri_scribe.report.gathering.make_entry("Bug")
...
def test_gather_report_uses_empty_scores_when_missing(
    ...
    fire_geometries = (
        tests.helpers.doubles.peri_scribe.report.gathering.make_scores_recorder(
...
def test_fire_identity_prefers_canonical_identifier() -> None:
    fire = tests.helpers.factories.peri_scribe.report.gathering.located_fire(
        "Bug",
        "2026-casnd-150541",
    )
...
def test_fire_location_measures_from_point_without_perimeter(
    ...
    nearest_city = (
        tests.helpers.doubles.peri_scribe.report.gathering
    ).make_city_measurement_recorder(
        measured=measured,
    )
...
def test_fire_locations_maps_each_located_fire_once(
    ...
    located = tests.helpers.factories.peri_scribe.report.gathering.located_fire(
        "Bug",
        "2026-casnd-150541",
    )
    without_perimeter = tests.helpers.factories.peri_scribe.report.gathering.make_fire(
        "Fire",
        "2026-casnd-150542",
    )
    nearest = peri_scribe.report.locations.NearestCity(
