...
test("loadUpdates checks HEAD every ten seconds", async () => {
  const page = await viewer.page(viewer.snapshot(), { ETag: '"content-hash"' });

  assert.equal(page.interval, 10000);
  await page.advance(9999);
  assert.deepEqual(page.calls.map(call => call.method), ["GET"]);
  await page.advance(1);
  assert.deepEqual(page.calls.map(call => call.method), ["GET", "HEAD"]);
  await page.advance(10000);
  assert.deepEqual(page.calls.map(call => call.method), ["GET", "HEAD", "HEAD"]);
});
...
test("loadUpdates recovers a same-second equal-length metadata collision", async () => {
  ...
  assert.equal(JSON.stringify(original).length, JSON.stringify(replacement).length);
  const headers = {
...
test("loadUpdates fetches a changed validator on the next check", async () => {
  ...
  assert.deepEqual(page.calls.map(call => call.method), ["GET", "HEAD", "GET"]);
  assert.equal(page.rendered.at(-1).updates[0].mapped_area.value, 2000);
});
...
test("loadUpdates asks the browser to bypass its cache and bound request waits",
  async () => {
    const page = await viewer.page(viewer.snapshot());
...
for (const [description, before, after] of [
  ...
  ["content-length was absent", { "Last-Modified": metadata["Last-Modified"] },
    metadata],
  ["content-length disappears", metadata,
    { "Last-Modified": metadata["Last-Modified"] }],
  ["last-modified changes", metadata,
    { ...metadata, "Last-Modified": "Wed, 23 Sep 2026 00:00:01 GMT" }],
  ["content-length changes", metadata, { ...metadata, "Content-Length": "401" }]
]) {
...
for (const [description, change] of [
  ...
  ["invalid generation time", data => ({ ...data, generated_at: "not a date" })],
...
for (const [description, change] of [
  ...
  ["incomplete log identity", record => ({ ...record, log_identity: ["name"] })],
  ["invalid log identity kind", record => ({
    ...record, log_identity: ["location", "Timber"]
  })],
  ["non-string log identity", record => ({ ...record, log_identity: ["name", 123] })],
  ...
  ["wrong acreage units", record => ({
    ...record, mapped_area: { value: 123, units: "hectare" }
  })],
  ["non-numeric acreage", record => ({
    ...record, mapped_area: { value: "123", units: "acre" }
  })],
  ["non-finite acreage", record => ({
    ...record, mapped_area: { value: Infinity, units: "acre" }
  })],
  ["negative acreage", record => ({
    ...record, mapped_area: { value: -123, units: "acre" }
  })],
  ["invalid previous acreage", record => ({
    ...record, previous_mapped_area: { value: -123, units: "acre" }
  })]
]) {
...
test("validateSnapshot accepts missing incident metadata and a zero acreage baseline",
  async () => {
...
for (const identity of [
  null, ["id", "2026-timber"], ["name", "Timber"], ["local", "new-timber"]
]) {
  test(`validateSnapshot accepts log identity ${JSON.stringify(identity)}`,
    async () => {
      const data = viewer.snapshot();
