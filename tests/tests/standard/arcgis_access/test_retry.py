"""Unit tests for arcgis_access.retry — rate-limit detection and backoff."""
...
import tests.helpers.doubles.arcgis_access.retry
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            arcgis_access.retry.FALLBACK_RETRY.m_as("seconds"),
        ),
    ],
)
def test_rate_limit_from_payload_ignores_details_without_retry_instructions(
    ...
    assert (
        arcgis_access.retry.rate_limit_from_payload({
...
def test_rate_limit_retry_preserves_json_retry_instruction(
    ...
    assert (
        arcgis_access.retry.rate_limit_retry(ValueError(text))
        == seconds * units.seconds
    )
...
def test_rate_limit_retry_accepts_error_text_without_an_envelope(
    ...
    assert (
        arcgis_access.retry.rate_limit_retry(RuntimeError(text))
        == seconds * units.seconds
    )


def test_rate_limit_retry_ignores_json_without_an_error_object() -> None:
    assert arcgis_access.retry.rate_limit_retry(ValueError("[]")) is None
...
def test_rate_limit_retry_uses_server_hint() -> None:
    ...
    assert (
        arcgis_access.retry.rate_limit_retry(error)
...
def test_rate_limit_retry_uses_fallback_for_loose_429() -> None:
    ...
    assert (
        arcgis_access.retry.rate_limit_retry(error)
        == arcgis_access.retry.FALLBACK_RETRY
    )


def test_rate_limit_retry_uses_server_hint_from_string() -> None:
    error = ValueError(
        tests.helpers.factories.arcgis_access.retry.RATE_LIMIT_ERROR_STRING,
    )
    assert (
        arcgis_access.retry.rate_limit_retry(error)
        == tests.helpers.factories.arcgis_access.retry.RATE_LIMIT_RETRY_AFTER
...
def test_rate_limit_retry_uses_fallback_for_loose_429_string() -> None:
    error = ValueError(
        tests.helpers.factories.arcgis_access.retry.LOOSE_429_ERROR_STRING,
    )
    assert (
        arcgis_access.retry.rate_limit_retry(error)
        == arcgis_access.retry.FALLBACK_RETRY
...
def test_rate_limit_retry_uses_retry_after_header() -> None:
    error = tests.helpers.factories.arcgis_access.retry.http_error(
        http.HTTPStatus.TOO_MANY_REQUESTS,
        retry_after=str(
            tests.helpers.factories.arcgis_access.retry.RETRY_AFTER_HEADER_IN_SECONDS,
        ),
    )
    assert (
        arcgis_access.retry.rate_limit_retry(error)
...
def test_rate_limit_retry_returns_none_for_other_http_errors() -> None:
    ...
    assert arcgis_access.retry.rate_limit_retry(error) is None
...
def test_is_transient_error_matches_transient_exception_types(
    ...
    assert arcgis_access.retry.is_transient_error(error) is True
...
def test_is_transient_error_does_not_match_normal_errors() -> None:
    ...
    assert arcgis_access.retry.is_transient_error(error) is False
...
def test_is_retryable_error_retries_rate_limit() -> None:
    ...
    assert arcgis_access.retry.is_retryable_error(error) is True
...
def test_is_retryable_error_rejects_other_errors() -> None:
    error = RuntimeError("boom")
    assert arcgis_access.retry.is_retryable_error(error) is False
...
def test_retry_reason_describes_rate_limit() -> None:
    ...
    assert (
        arcgis_access.retry.retry_reason(error)
...
def test_retry_wait_uses_server_hint() -> None:
    retry_state = tests.helpers.factories.arcgis_access.retry.failed_retry_state(
        ValueError(
            tests.helpers.factories.arcgis_access.retry.RATE_LIMIT_ERROR_PAYLOAD,
        ),
    )
    assert arcgis_access.retry.retry_wait(
        retry_state,
    ) == tests.helpers.factories.arcgis_access.retry.RATE_LIMIT_RETRY_AFTER.m_as(
        "second",
    )
...
def test_retry_wait_uses_fallback_for_loose_429() -> None:
    retry_state = tests.helpers.factories.arcgis_access.retry.failed_retry_state(
        ValueError(tests.helpers.factories.arcgis_access.retry.LOOSE_429_ERROR_PAYLOAD),
    )
    assert arcgis_access.retry.retry_wait(
        retry_state,
    ) == arcgis_access.retry.FALLBACK_RETRY.m_as("seconds")
...
def test_retry_wait_uses_exponential_backoff() -> None:
    ...
    assert arcgis_access.retry.retry_wait(retry_state) == pytest.approx(
        arcgis_access.retry.BACKOFF_BASE.m_as("seconds") * 4,
    )
...
def test_retry_wait_caps_backoff_at_maximum() -> None:
    ...
    assert arcgis_access.retry.retry_wait(
        retry_state,
    ) == arcgis_access.retry.BACKOFF_MAXIMUM.m_as("seconds")


def test_last_error_returns_failed_exception() -> None:
    error = ValueError("boom")
    assert (
        arcgis_access.retry.last_error(
            tests.helpers.factories.arcgis_access.retry.failed_retry_state(error),
        )
        is error
    )
...
def test_last_error_raises_without_outcome() -> None:
    ...
    with pytest.raises(AssertionError, match="failed attempt"):
        arcgis_access.retry.last_error(retry_state)
...
def test_run_with_retry_logs_serializable_traceback_on_exhaustion(
    ...
    failing_query = tests.helpers.doubles.arcgis_access.retry.raise_disconnected_query

    with pytest.raises(requests.exceptions.ConnectionError, match="Disconnected"):
        arcgis_access.retry.run_with_retry("example", failing_query, maximum_retries=0)
...
def test_run_with_retry_logs_traceback_before_a_successful_retry(
    ...
    assert arcgis_access.retry.run_with_retry("example", query) == "result"
