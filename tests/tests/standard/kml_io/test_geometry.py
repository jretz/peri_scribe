...
def test_kml_writer_document_preserves_names_styles_and_description(
    ...
    with writer.document(
        ...
        writer.write("<Placemark><name>River</name></Placemark>")
    ...
    assert document.findtext(tag("name")) == "Cities & rivers 日本"
    ...
    assert document.findtext(f"{tag('Placemark')}/{tag('name')}") == "River"


def test_kml_writer_document_closes_envelope_after_renderer_failure() -> None:
    writer = tests.helpers.factories.kml_io.geometry.MemoryKmlWriter()
    message = "rendering failed"
    with (
        pytest.raises(ValueError, match="rendering failed"),
        writer.document("Map", ()),
    ):
...
def test_ring_coordinates_text_rounds_and_omits_altitude() -> None:
    ...
    assert (
        kml_io.geometry.ring_coordinates_text(ring)
...
def test_polygon_geometry_includes_holes() -> None:
    ...
    writer = tests.helpers.factories.kml_io.geometry.MemoryKmlWriter()
    kml_io.geometry.polygon_geometry(
        writer,
        "Bug",
        "#perimeter-fill",
        polygon,
...
def test_multi_polygon_geometry_holds_each_polygon() -> None:
    ...
    writer = tests.helpers.factories.kml_io.geometry.MemoryKmlWriter()
    kml_io.geometry.multi_polygon_geometry(
        writer,
        "Bug",
        "#perimeter-fill",
        multi_polygon,
    ...
    assert tests.helpers.kml_io.geometry.polygon_draw_orders(geometry) == [
        expected_draw_order,
        expected_draw_order,
    ]
    assert (
