...
Tests must not write to the repository or depend on persistent local state. Code that
intentionally reads or writes files should use pytest's per-test `tmp_path` directory or
an equivalent isolated temporary location; network access remains mocked.
